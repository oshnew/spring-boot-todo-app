...
public class TodoApiController {
    ...
    public ResponseEntity<ResData<TodoEntity>> get(@Valid TodoDTO.Get param, BindingResult result) {
        if (result.hasErrors()) {
            return new ResponseEntity<>(new ResData<>(null, CmnUtils.getErrMsg(result, "<br>")), HttpStatus.BAD_REQUEST);
        }
        ...
        try {
            ...
            return new ResponseEntity<>(new ResData<>(null, "시스템에러가 발생했습니다."), HttpStatus.INTERNAL_SERVER_ERROR);
        }
